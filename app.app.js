...
/* === Amortización: saldo pendiente después de k pagos ===
   Fórmula estándar (francés):
   saldo_k = M * ((1+i)^n - (1+i)^k) / ((1+i)^n - 1)
   Si i = 0 -> lineal.
*/
function saldoPendiente(M, i, n, k) {
  k = Math.max(0, Math.min(n, k || 0));
  if (!n || n <= 0) return 0;
  if (!i) return Math.max(0, M * (1 - k / n));
  const f = Math.pow(1 + i, n);
  const fk = Math.pow(1 + i, k);
  return Math.max(0, M * (f - fk) / (f - 1));
}
...
class Finanzas {
  ...
  bindUI() {
    ...
    // Delegación: editar / eliminar / añadir ahorro / pagar
  ...
  /** Duplica mes anterior. En deudas:
   * - Nuevo mes inicia con pagadoMes = false.
   * - Si el mes previo estaba pagadoMes=true, incrementa cuotasPagadas en el nuevo mes.
   * En gastos/ingresos/compras: copia y resetea pagado=false.
   */
  ensureMonth(key) {
    ...
    if (this.data[prev]) {
      const src = this.data[prev];
      const copy = {
        ingresos: (src.ingresos || []).map(x => ({
          ...x, id: this.uid(), fecha: `${key}-01`, pagado: false
        })),
        gastosFijos: (src.gastosFijos || []).map(x => ({
          ...x, id: this.uid(), fecha: `${key}-01`, pagado: false
        })),
        tarjetas: (src.tarjetas || []).map(x => ({
          ...x,
          id: this.uid(),
          fecha: `${key}-01`,
          // si el mes anterior estuvo marcado pagadoMes, avanzamos 1
          cuotasPagadas: Math.min((x.numeroCuotas||0), (x.cuotasPagadas||0) + (x.pagadoMes ? 1 : 0)),
          pagadoMes: false
        })),
        creditos: (src.creditos || []).map(x => ({
          ...x,
          id: this.uid(),
          fecha: `${key}-01`,
          cuotasPagadas: Math.min((x.numeroCuotas||0), (x.cuotasPagadas||0) + (x.pagadoMes ? 1 : 0)),
        ...
        gastosCompras: (src.gastosCompras || []).map(x => ({
          ...x, id: this.uid(), fecha: `${key}-01`, pagado: false
        })),
        ahorros: (src.ahorros || []).map(x => ({
          ...x, id: this.uid(), fecha: `${key}-01`
        }))
      };
      this.data[key] = copy;
      // Recalcular cuotas mensuales por si cambió cuotasPagadas
      this.recalcDeudas(this.data[key]);
      this.save();
    } else {
      this.data[key] = {
        ingresos: [],
  ...
  renderAll() {
    ...
    // Gastos fijos y compras solo cuentan si NO están marcados pagados? → No: siguen siendo gasto del mes igual.
    const totalFix = d.gastosFijos.reduce((s, x) => s + (x.monto || 0), 0);
    const totalCom = d.gastosCompras.reduce((s, x) => s + (x.monto || 0), 0);

    // En deudas, el gasto del mes es la cuota (independiente de marcar pagado,
    // marcar es solo control visual y para copiar al mes siguiente).
  ...
  rowGeneric(icon, i, key, monto) {
    const paid = i.pagado ? `<span class="badge success">pagado</span>` : "";
    return `<div class="item ${i.pagado ? "is-paid": ""}">
      <div class="row">
        <div>${icon} <b>${i.nombre}</b> ${paid}
          <div class="meta">${i.categoria || "General"} · ${i.fecha || ""}</div>
        </div>
        <div><b>${fmt(monto)}</b></div>
      </div>
      <div class="actions">
        <a class="pill paid" data-action="paid" data-key="${key}" data-id="${i.id}" href="#">${i.pagado?"↩️ Desmarcar pago":"✅ Pagado este mes"}</a>
        <a class="pill" data-action="edit" data-key="${key}" data-id="${i.id}" href="#">✏️ Editar</a>
        <a class="pill" data-action="del" data-key="${key}" data-id="${i.id}" href="#">🗑️ Eliminar</a>
      </div>
    </div>`;
  }

  rowTarjeta(i, key) {
    const saldo = Math.round(saldoPendiente(
      Number(i.montoTotal||0), Number(i.tasaMensual||0), parseInt(i.numeroCuotas||0), parseInt(i.cuotasPagadas||0)
    ));
    const paid = i.pagadoMes ? `<span class="badge success">cuota pagada</span>` : "";
    return `<div class="item ${i.pagadoMes ? "is-paid": ""}">
      <div class="row">
        <div>💳 <b>${i.nombre}</b> ${paid}
          <div class="meta">
      ...
      <div class="actions">
        <a class="pill paid" data-action="paid" data-key="${key}" data-id="${i.id}" href="#">${i.pagadoMes?"↩️ Desmarcar pago":"✅ Pagado este mes"}</a>
        <a class="pill" data-action="edit" data-key="${key}" data-id="${i.id}" href="#">✏️ Editar</a>
        <a class="pill" data-action="del" data-key="${key}" data-id="${i.id}" href="#">🗑️ Eliminar</a>
      </div>
    </div>`;
  ...
  rowCredito(i, key) {
    ...
    const extras = [
      ...
      i.ivaAvalPct?`IVA aval ${formatPctComma(i.ivaAvalPct)}%`:""
    ].filter(Boolean).join(" · ");
  ...
  rowAhorro(i, key) {
    ...
    return `<div class="item">
      ...
      <div class="actions">
        <a class="pill" data-action="addsave" data-id="${i.id}" href="#">💰 Añadir</a>
        <a class="pill" data-action="edit" data-key="${key}" data-id="${i.id}" href="#">✏️ Editar</a>
        <a class="pill" data-action="del" data-key="${key}" data-id="${i.id}" href="#">🗑️ Eliminar</a>
      </div>
    </div>`;
  ...
  togglePagado(key, id) {
    ...
    if (key === "tarjetas" || key === "creditos") {
      // Alterna pago de la cuota del mes
      if (!it.pagadoMes) {
        // marcar como pagado: sube cuotasPagadas (sin superar el total)
        it.pagadoMes = true;
        it.cuotasPagadas = Math.min((it.numeroCuotas||0),(it.cuotasPagadas||0)+1);
      } else {
        // desmarcar: baja cuotasPagadas (sin caer por debajo de 0)
        it.pagadoMes = false;
        it.cuotasPagadas = Math.max(0,(it.cuotasPagadas||0)-1);
      }
    } else {
      // fijos, ingresos, compras: solo una marca visual del mes
      it.pagado = !it.pagado;
    }

    this.save();
    this.renderAll();
    this.toast(it.pagado || it.pagadoMes ? "Marcado como pagado" : "Pago desmarcado");
  }
  ...
  openForm(tipo, item = null) {
    ...
    this.showModal(title, fields, (vals) => {
      ...
      if (tipo === "ingreso") {
        d.ingresos.push({ id: this.uid(), nombre: vals.nombre, monto: n(vals.monto), categoria: vals.categoria, fecha: vals.fecha, pagado:false });
      } else if (tipo === "fijo") {
        d.gastosFijos.push({ id: this.uid(), nombre: vals.nombre, monto: n(vals.monto), categoria: vals.categoria, fecha: vals.fecha, pagado:false });
      } else if (tipo === "compra") {
        d.gastosCompras.push({ id: this.uid(), nombre: vals.nombre, monto: n(vals.monto), categoria: vals.categoria, fecha: vals.fecha, pagado:false });
      } else if (tipo === "ahorro") {
  ...
  showModal(title, innerHtml, onSubmit) {
    ...
    form.innerHTML = innerHtml + `<div class="actions"><button type="submit" class="primary">Guardar</button><button type="button" class="cancel" id="cancelModal">Cancelar</button></div>`;
  ...
  toast(m) {
    const t = this.toastEl; if (!t) return; t.textContent = m; t.classList.add("show"); setTimeout(() => t.classList.remove("show"), 1800);
  }
}
...
(function () {
  function safeInit() {
    ...
    } catch (err) { console.error("Safe init error:", err); }
  }
