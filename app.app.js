...
class Finanzas {
  ...
  recalcDeudas(d){
    (d.tarjetas||[]).forEach(it=>{
      ...
      // Normalizar cuota de manejo si no existe
      if(typeof it.manejoMensual!=="number") it.manejoMensual=0;
    });
  ...
  renderAll(){
    ...
    const totalTar = d.tarjetas.reduce((s,x)=>s+(x.cuotaMensual||0)+(x.manejoMensual||0),0);
  ...
  rowTarjeta(i,key){
    const paidClass = i.paid ? "is-paid" : "";
    const totalMensual = (i.cuotaMensual || 0) + (i.manejoMensual || 0);
    return `<div class="item ${paidClass}">
      <div class="row">
        <div>💳 <b>${i.nombre}</b>
          <div class="meta">Cuota ${fmt(i.cuotaMensual)}${(i.manejoMensual||0)?` + manejo ${fmt(i.manejoMensual)}`:""} · ${i.cuotasPagadas||0}/${i.numeroCuotas} · tasa ${formatPctComma(i.tasaMensual)}%</div>
        </div>
        <div><b>Total ${fmt(i.montoTotal)}</b><div class="meta">Mensual: ${fmt(totalMensual)}</div></div>
      </div>
      <div class="actions">
        <a data-action="edit" data-key="${key}" data-id="${i.id}" href="#">✏️ Editar</a>
        <a data-action="del" data-key="${key}" data-id="${i.id}" href="#">🗑️ Eliminar</a>
  ...
  openForm(tipo,item=null){
    ...
    if(tipo==="ingreso"){
      ...
      fields= f("nombre","text","Nombre","")
            ...
            + f("manejo","number","Cuota de manejo mensual","0","step='1' min='0'");
    ...
    this.showModal(title, fields, (vals)=>{
      ...
      if(tipo==="ingreso"){
        ...
        d.tarjetas.push({id:this.uid(),nombre:vals.nombre,montoTotal:M,numeroCuotas:cu,cuotasPagadas:pag,tasaMensual:tasa,cuotaMensual:cuota,manejoMensual:n(vals.manejo),fecha:"01",paid:false});
      }else if(tipo==="credito"){
  ...
  edit(key,id){
    ...
    if(!isDeuda && key!=="ahorros"){
      ...
      fields= f("nombre","text","Nombre",it.nombre)
            ...
            + f("manejo","number","Cuota de manejo mensual",(it.manejoMensual||0),"step='1' min='0'");
    ...
    this.showModal(title, fields, (vals)=>{
      ...
      if(!isDeuda && key!=="ahorros"){
        ...
        const M=n(vals.montoTotal), cu=parseInt(vals.numeroCuotas||0), pag=parseInt(vals.cuotasPagadas||0);
        Object.assign(it,{nombre:vals.nombre,montoTotal:M,numeroCuotas:cu,cuotasPagadas:pag,tasaMensual:tasa,cuotaMensual:this.cuota(M,tasa,cu),manejoMensual:n(vals.manejo)});
      }else if(key==="creditos"){
        const tasa=pct(vals.tasa), aval=pct(vals.aval||"0"), iva=pct(vals.ivaAval||"0");
